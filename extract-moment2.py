...
for a in tf:
    ...
    sp = b.split('/'); path = '/'.join(sp[:-1])+'_'+sp[-1]
    allpath.append(path)
    #break
...
def _extract_eig(
    ...
    # Eigenvectors of matting laplacian matrix
        ...
        if image_color_lambda > 0:
            ...
            patch_size=16
            #path = 'data/VOC2012/ytb_images_train/'+name.split('/')[-1].replace('.pth','.jpg')
            #path = 'data/VOC2012/ytb_images_train/5ef5860ac6_00050.jpg'
            #path = 'data/VOC2012/ytb_images_train/0693719753_00050.jpg'

            gray_image = cv2.imread(image_file);

            num_rows = gray_image.shape[0] // patch_size
            num_cols = gray_image.shape[1] // patch_size
            patches1 = extract_patches(gray_image[:,:,0], patch_size)
            patches2 = extract_patches(gray_image[:,:,1], patch_size)
            patches3 = extract_patches(gray_image[:,:,2], patch_size)

            #entropy = calculate_entropy(patches,num_rows,num_cols)

            if moment=='kor':
                resmom1 = calculate_kurtosis(patches1,num_rows,num_cols)
                resmom2 = calculate_kurtosis(patches2,num_rows,num_cols)
                resmom3 = calculate_kurtosis(patches3,num_rows,num_cols)
            elif moment=='std':
                resmom1 = calculate_std(patches1,num_rows,num_cols)
                resmom2 = calculate_std(patches2,num_rows,num_cols)
                resmom3 = calculate_std(patches3,num_rows,num_cols)
            elif moment=='skew':
            ...
            #image_lr = resmom.reshape(-1,1)

            image_lr = np.stack((resmom1,resmom2,resmom3), axis=2)
            #print('\n @@@@@@@@@@@ image_lr',image_lr.shape)
            ...
            '''
            image_lr = (image_lr @ image_lr.T)
            if threshold_at_zero:
                image_lr = (image_lr * (image_lr > 0))
            W_color = image_lr / image_lr.max()  # NOTE: If features are normalized, this naturally does nothing
            '''
    ...
    torch.save(output_dict, output_file)
        #print('in',features_file)
    #else:
    #    print('not in',features_file)
...
def extract_eigs(
    ...
    kwargs = dict(K=K, which_matrix=which_matrix, which_features=which_features, which_color_matrix=which_color_matrix,
                 ...
                 image_downsample_factor=image_downsample_factor, image_color_lambda=image_color_lambda, lapnorm=lapnorm,moment=moment)
    print(kwargs)
...
def extract_bbox_clusters(
    ...
    #print(torch.norm(all_features, dim=-1, keepdim=True).size())
    ...
    #with open('Kmeans_cnt.obj', 'wb') as fp:
...
def extract_semantic_segmentations(
    ...
    total_num_boxes = sum(len(d['bboxes']) for d in bbox_list)
    print('Loaded bounding box list. There are',total_num_boxes,'total bounding boxes with features and clusters.')
...
def extract_crf_segmentations(
    ...
    print('Found,',len(inputs),'images and segmaps')
    utils.parallel_process(inputs, fn, multiprocessing)
